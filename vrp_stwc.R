...
tourLength <- function(tour, solomon_mat) {
  ...
  for(i in 1:route_length)
  {
    if(vehicle_count <= 25)
    {
      ...
      if(arrival_time[route[i,2]] >= solomon$READY.TIME[route[i,2]] & depart_time <= solomon$DUE.DATE[route[i,2]]){
        tourlength = tourlength + 0
      }
      else
      {
        flag <- 1
  ...
  #-------------End : Strict Time Window Constraint-------------------
...
vrp_WS_STWC_fitness_function <- function(tour, ...){
